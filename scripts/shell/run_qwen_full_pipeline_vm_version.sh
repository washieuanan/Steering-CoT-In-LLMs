...
#    --run_dir "results/phase_a/Qwen2.5-7B-Instruct__qwen_causality_v3/mmlu_pro"\
#    --judge_model "gpt-5-nano"
#
## =============================================================
...
for LOCALITY in cot answer; do
    # Generate a single tag for this locality run - all modes will share the same directory
    PHASE_B_TAG="${LOCALITY}_locality_$(date +%Y%m%d_%H%M%S)"
    echo ">>> Phase B tag for ${LOCALITY}: ${PHASE_B_TAG}"
    echo ""
    echo "###################################################"
